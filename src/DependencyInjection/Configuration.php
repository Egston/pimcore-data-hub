            ...
            ->children()
                ->arrayNode('graphql')
                    ->children()
                        ...
                        ->booleanNode('thumbnail_deferred_default')->info('default for the asset-thumbnail `deferred` resolver argument when a query omits it. false (upstream) generates the thumbnail eagerly at resolve time so the returned URL points to a ready file; true returns a deferred URL and lets the web server generate it on-demand on first request — recommended when GraphQL is resolved by a background/SWR refresh worker, so eager Imagick generation does not run (and OOM) in the worker. A per-query `deferred` argument always overrides this.')->defaultValue(false)->end()
                ...
                ->arrayNode('request_validation')
                    ->info('versioned default-deny request-variable validation for the GraphQL endpoint. A no-op until a rules file is mounted AND a client is listed in enforced_clients.')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('rules_file')
                            ->info('absolute path to the JSON rules file (mounted from a ConfigMap directory). Empty disables the engine.')
                            ->defaultValue('')
                        ->end()
                        ->arrayNode('enforced_clients')
                            ->info('datahub configuration (client) names the validator enforces. Empty enforces no client.')
                            ->scalarPrototype()->end()
                            ->defaultValue([])
                        ->end()
                    ->end()

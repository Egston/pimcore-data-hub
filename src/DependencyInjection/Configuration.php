                ...
                ->arrayNode('graphql')
                    ->children()
                        ...
                        ->arrayNode('in_progress_queries')
                            ...
                            ->defaultValue([])
                        ->end()
                        ->integerNode('in_progress_ttl')->info('TTL in seconds for the in-progress marker stored in cache')->defaultValue(60)->end()
                        ...
                        ->end()
                        ->booleanNode('in_progress_enable_by_param')
                            ->info('if true, protection is enabled only when the request contains the configured query parameter set to true')
                            ->defaultFalse()
                        ->end()
                        ->scalarNode('in_progress_param_name')
                            ->info('name of the query parameter that enables in-progress protection when present and truthy')
                            ->defaultValue('datahub_guard')
                        ->end()
